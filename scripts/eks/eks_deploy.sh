...
build_distribution() {
  (cd "$ROOT_DIR" && ./gradlew :node:installDist :control-plane:installDist --no-daemon)
}
...
kubectl -n "$NAMESPACE" set env statefulset/notdynamo-data \
  ...
  NOTDYNAMO_RATIS_PORT="10090" \
  NOTDYNAMO_RATIS_GROUP_NAME="notdynamo-main" \
  NOTDYNAMO_RATIS_REQUEST_TIMEOUT_MS="5000" \
