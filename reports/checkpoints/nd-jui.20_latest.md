...
- Timestamp (UTC): `2026-02-18 22:32:56Z`
- Issue: `nd-jui.20`
- Claimed now: `0`
- Status override: `none`
- Closed now: `1`
- Note: `Implemented scripts/eks/eks_bench_nodegroup_up.sh plus benchmark node label/taint options in bench job, matrix, scaling sweep, lockstep sweep, and runbook; validated split-infra run with benchmark pods pinned to dedicated nodes and improved p95/p99 tail latency`

## Issue Snapshot

```text
✓ nd-jui.20 · E41 Benchmark infrastructure isolation (dedicated nodegroup)   [● P1 · CLOSED]
...
DESCRIPTION
Separate benchmark generators from service pods by introducing a dedicated EKS benchmark nodegroup with explicit label/taint scheduling controls in benchmark scripts and runbook flows.

NOTES
[2026-02-18 22:32:55Z] Start E41: dedicated benchmark nodegroup isolation and benchmark job scheduling controls
[2026-02-18 22:32:56Z] Implemented scripts/eks/eks_bench_nodegroup_up.sh plus benchmark node label/taint options in bench job, matrix, scaling sweep, lockstep sweep, and runbook; validated split-infra run with benchmark pods pinned to dedicated nodes and improved p95/p99 tail latency

ACCEPTANCE CRITERIA
Benchmark jobs can be pinned to dedicated benchmark nodes; report artifacts capture node placement and scheduling metadata; service pods remain on service nodegroup during runs.

LABELS: benchmark, eks, single-az
...
5. [● P1] [task] nd-jui.21: E42 Benchmark saturation telemetry pack
