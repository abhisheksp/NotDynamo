...
- Timestamp (UTC): `2026-02-19 07:57:11Z`
- Issue: `nd-jui.21`
- Claimed now: `0`
- Status override: `none`
- Closed now: `1`
- Note: `Validated telemetry pack live on EKS with reports/benchmarks/aws/e2e_http_incluster_e42_validation_20260219T075408Z.json and run dir reports/benchmarks/aws/incluster_runs/notdynamo-bench-http-20260219075411 (pod_placement + pods/nodes top snapshots + group summaries + attribution signals). Added telemetry propagation into benchmark matrix/scaling/lockstep summaries and in-cluster image reuse flags to avoid per-run rebuilds.`

## Issue Snapshot

```text
✓ nd-jui.21 · E42 Benchmark saturation telemetry pack   [● P1 · CLOSED]
...
DESCRIPTION
Standardize benchmark-time telemetry capture (pod/node CPU-memory snapshots, pod placement, and error counters) so each run has enough evidence to attribute bottlenecks.

NOTES
[2026-02-18 22:48:21Z] Start E42: implement standardized telemetry capture/summary per benchmark run for generator-vs-service bottleneck attribution.
[2026-02-19 06:16:14Z] Implemented telemetry pack in scripts/eks/eks_bench_job_up.sh: per-run pod placement + pods/nodes top snapshots + group summaries under incluster run dir; added normalized attribution signals (generator/service cpu ratio + hint) into JSON/Markdown reports and surfaced telemetry summary in benchmark matrix. Validation blocked by expired AWS session; pending live EKS run after aws login.
[2026-02-19 07:57:11Z] Validated telemetry pack live on EKS with reports/benchmarks/aws/e2e_http_incluster_e42_validation_20260219T075408Z.json and run dir reports/benchmarks/aws/incluster_runs/notdynamo-bench-http-20260219075411 (pod_placement + pods/nodes top snapshots + group summaries + attribution signals). Added telemetry propagation into benchmark matrix/scaling/lockstep summaries and in-cluster image reuse flags to avoid per-run rebuilds.

ACCEPTANCE CRITERIA
Each benchmark run emits telemetry artifacts and a normalized summary section with generator saturation vs service saturation signals.

LABELS: benchmark, observability, single-az

DEPENDS ON
  → ✓ nd-jui.20: E41 Benchmark infrastructure isolation (dedicated nodegroup) ● P1

BLOCKS
  ← ○ nd-jui.22: E43 Lockstep rerun at n=11/29/35 on isolated benchmark infra ● P1
...
6. [● P2] [task] nd-jui.19: E40 Reproducible benchmark runbook and report index
   Assignee: Abhishek Srinivasa Raju Padmavathi
7. [● P1] [task] nd-jui.22: E43 Lockstep rerun at n=11/29/35 on isolated benchmark infra
