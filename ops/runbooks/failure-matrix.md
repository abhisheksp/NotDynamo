...
| Node drain live recovery drill | Cordon + drain worker node + recovery smoke | `./scripts/local/kind_failure_node_drain.sh --namespace notdynamo` | Cluster recovers scheduling and cross-node PUT/GET/DELETE still succeeds |
...
- `./scripts/verify/g18.sh` (aggregate pre-benchmark correctness gate)
